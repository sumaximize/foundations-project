...
SameSite: t && t.SameSite || s.defaults.SameSite,
...
o += n.SameSite ? ";SameSite=" + n.SameSite : "", o += n.expires ? ";expires=" + n.expires.toUTCString() : "", 
o += n.secure ? ";secure" : "";
...
}("undefined" == typeof window ? this : window), window.addEventListener("load", function() {
var t, e, n;
...
var _, v = 1;
if (h) {
if (!(_ = L(g, y, t, p)) || _.index >= t.length) break;
var E = _.index, w = _.index + _[0].length, S = y;
for (S += m.value.length; S <= E; ) S += (m = m.next).value.length;
if (y = S -= m.value.length, m.value instanceof D) continue;
...
var N = m.prev;
if (k && (N = P(e, N, k), y += k.length), x(e, N, v), m = P(e, N, new D(i, u ? M.tokenize(C, u) : C, d, C)), 
I && P(e, m, I), 1 < v) {
var R = {
cause: i + "," + c,
reach: A
};
O(t, e, n, m.prev, y, R), s && R.reach > s.reach && (s.reach = R.reach);
}
}
}
}
...
var n = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, e = 0, o = {}, M = {
manual: l.Prism && l.Prism.manual,
disableWorkerMessageHandler: l.Prism && l.Prism.disableWorkerMessageHandler,
util: {
encode: function c(t) {
return t instanceof D ? new D(t.type, c(t.content), t.alias) : Array.isArray(t) ? t.map(c) : t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/\u00a0/g, " ");
},
type: function(t) {
...
clone: function u(t, n) {
var o, e;
switch (n = n || {}, M.util.type(t)) {
case "Object":
if (e = M.util.objId(t), n[e]) return n[e];
for (var r in o = {}, n[e] = o, t) t.hasOwnProperty(r) && (o[r] = u(t[r], n));
return o;
...
t.className = t.className.replace(RegExp(n, "gi"), ""), t.classList.add("language-" + e);
},
...
if (M.hooks.run("before-tokenize", o), !o.grammar) throw new Error('The language "' + o.language + '" has no grammar.');
return o.tokens = M.tokenize(o.code, o.grammar), M.hooks.run("after-tokenize", o), 
D.stringify(M.util.encode(o.tokens), o.language);
},
...
if (l.Prism = M, D.stringify = function h(t, e) {
...
if (r && (M.filename = r.src, r.hasAttribute("data-manual") && (M.manual = !0)), 
!M.manual) {
var a = document.readyState;
"loading" === a || "interactive" === a && r && r.defer ? document.addEventListener("DOMContentLoaded", t) : window.requestAnimationFrame ? window.requestAnimationFrame(t) : window.setTimeout(t, 16);
}
return M;
}(_self);
...
var n = "(?:" + [ /([^a-zA-Z0-9\s{(\[<=])(?:(?!\1)[^\\]|\\[\s\S])*\1/.source, /\((?:[^()\\]|\\[\s\S]|\((?:[^()\\]|\\[\s\S])*\))*\)/.source, /\{(?:[^{}\\]|\\[\s\S]|\{(?:[^{}\\]|\\[\s\S])*\})*\}/.source, /\[(?:[^\[\]\\]|\\[\s\S]|\[(?:[^\[\]\\]|\\[\s\S])*\])*\]/.source, /<(?:[^<>\\]|\\[\s\S]|<(?:[^<>\\]|\\[\s\S])*>)*>/.source ].join("|") + ")", o = /(?:"(?:\\.|[^"\\\r\n])*"|(?:\b[a-zA-Z_]\w*|[^\s\0-\x7F]+)[?!]?|\$.)/.source;
...
string: [ {
pattern: RegExp(/\b(?:q|qq|qw|qx)(?![a-zA-Z0-9])\s*/.source + "(?:" + [ /([^a-zA-Z0-9\s{(\[<])(?:(?!\1)[^\\]|\\[\s\S])*\1/.source, /([a-zA-Z0-9])(?:(?!\2)[^\\]|\\[\s\S])*\2/.source, e ].join("|") + ")"),
greedy: !0
}, {
pattern: /("|`)(?:(?!\1)[^\\]|\\[\s\S])*\1/,
greedy: !0
...
regex: [ {
pattern: RegExp(/\b(?:m|qr)(?![a-zA-Z0-9])\s*/.source + "(?:" + [ /([^a-zA-Z0-9\s{(\[<])(?:(?!\1)[^\\]|\\[\s\S])*\1/.source, /([a-zA-Z0-9])(?:(?!\2)[^\\]|\\[\s\S])*\2/.source, e ].join("|") + ")" + /[msixpodualngc]*/.source),
greedy: !0
}, {
pattern: RegExp(/(^|[^-])\b(?:s|tr|y)(?![a-zA-Z0-9])\s*/.source + "(?:" + [ /([^a-zA-Z0-9\s{(\[<])(?:(?!\2)[^\\]|\\[\s\S])*\2(?:(?!\2)[^\\]|\\[\s\S])*\2/.source, /([a-zA-Z0-9])(?:(?!\3)[^\\]|\\[\s\S])*\3(?:(?!\3)[^\\]|\\[\s\S])*\3/.source, e + /\s*/.source + e ].join("|") + ")" + /[msixpodualngcer]*/.source),
lookbehind: !0,
greedy: !0
}, {
...
Prism.languages.py = Prism.languages.python, function(t) {
for (var e = /"(?:\\.|[^\\"\r\n])*"|'(?:\\.|[^\\'\r\n])*'/.source, n = /\/\/.*(?!.)|\/\*(?:[^*]|\*(?!\/))*\*\//.source, o = /(?:[^\\()[\]{}"'/]|<string>|\/(?![*/])|<comment>|\(<expr>*\)|\[<expr>*\]|\{<expr>*\}|\\[\s\S])/.source.replace(/<string>/g, function() {
return e;
}).replace(/<comment>/g, function() {
return n;
}), r = 0; r < 2; r++) o = o.replace(/<expr>/g, function() {
return o;
});
...
r = t(r).source, i.languages.jsx = i.languages.extend("markup", e), i.languages.jsx.tag.pattern = t(/<\/?(?:[\w.:-]+(?:<S>+(?:[\w.:$-]+(?:=(?:"(?:\\[\s\S]|[^\\"])*"|'(?:\\[\s\S]|[^\\'])*'|[^\s{'"/>=]+|<BRACES>))?|<SPREAD>))*<S>*\/?)?>/.source), 
i.languages.jsx.tag.inside.tag.pattern = /^<\/?[^\s>\/]*/, i.languages.jsx.tag.inside["attr-value"].pattern = /=(?!\{)(?:"(?:\\[\s\S]|[^\\"])*"|'(?:\\[\s\S]|[^\\'])*'|[^\s'">]+)/, 
...
}), Prism.languages.scss.atrule.inside.rest = Prism.languages.scss, Prism.languages.scala = Prism.languages.extend("java", {
"triple-quoted-string": {
pattern: /"""[\s\S]*?"""/,
greedy: !0,
alias: "string"
...
}), t.languages.typescript.keyword.push(/\b(?:abstract|declare|is|keyof|readonly|require)\b/, /\b(?:asserts|infer|interface|module|namespace|type)\b(?=\s*(?:[{_$a-zA-Z\xA0-\uFFFF]|$))/, /\btype\b(?=\s*(?:[\{*]|$))/), 
...
var n = /\b(?:align|allowzero|and|anyframe|anytype|asm|async|await|break|cancel|catch|comptime|const|continue|defer|else|enum|errdefer|error|export|extern|fn|for|if|inline|linksection|nakedcc|noalias|nosuspend|null|or|orelse|packed|promise|pub|resume|return|stdcallcc|struct|suspend|switch|test|threadlocal|try|undefined|union|unreachable|usingnamespace|var|volatile|while)\b/, o = "\\b(?!" + n.source + ")(?!\\d)\\w+\\b", r = /align\s*\((?:[^()]|\([^()]*\))*\)/.source, s = "(?!\\s)(?:!?\\s*(?:" + /(?:\?|\bpromise->|(?:\[[^[\]]*\]|\*(?!\*)|\*\*)(?:\s*<ALIGN>|\s*const\b|\s*volatile\b|\s*allowzero\b)*)/.source.replace(/<ALIGN>/g, t(r)) + "\\s*)*" + /(?:\bpromise\b|(?:\berror\.)?<ID>(?:\.<ID>)*(?!\s+<ID>))/.source.replace(/<ID>/g, t(o)) + ")+";
e.languages.zig = {
comment: [ {
pattern: /\/\/[/!].*/,
alias: "doc-comment"
...
pattern: RegExp(/(:\s*)<TYPE>(?=\s*(?:<ALIGN>\s*)?[=;,)])|<TYPE>(?=\s*(?:<ALIGN>\s*)?\{)/.source.replace(/<TYPE>/g, t(s)).replace(/<ALIGN>/g, t(r))),
lookbehind: !0,
inside: null
}, {
pattern: RegExp(/(\)\s*)<TYPE>(?=\s*(?:<ALIGN>\s*)?;)/.source.replace(/<TYPE>/g, t(s)).replace(/<ALIGN>/g, t(r))),
lookbehind: !0,
inside: null
} ],
"builtin-type": {
...
var l = x(6), o = x(7), c = x(1), s = x(5), a = s.isError, u = s.isObject, p = s.isErrorEvent, h = s.isUndefined, d = s.isFunction, f = s.isString, g = s.isArray, i = s.isEmptyObject, m = s.each, y = s.objectMerge, b = s.truncate, _ = s.objectFrozen, v = s.hasKey, E = s.joinRegExp, w = s.urlencode, S = s.uuid4, T = s.htmlTreeAsString, C = s.isSameException, k = s.isSameStacktrace, I = s.parseUrl, A = s.fill, N = x(2).wrapMethod, R = "source protocol user pass host port path".split(" "), D = /^(?:(\w+):)?\/\/(?:(\w+)(:\w+)?@)?([\w\.-]+)(?::(\d+))?(\/.*)/, L = "undefined" != typeof window ? window : void 0 !== t ? t : "undefined" != typeof self ? self : {}, O = L.document, P = L.navigator;
e.prototype = {
VERSION: "3.20.1",
debug: !1,
TraceKit: l,
...
var t, e, n, o;
if ("production" === app.config.env && "1" !== navigator.doNotTrack && !navigator.globalPrivacyControl) if (e = Cookies.get("analyticsConsent"), 
t = Cookies.get("analyticsConsentAsked"), "1" === e) for (n = 0, o = h.length; n < o; n++) h[n].call(); else void 0 === e && void 0 === t && (Cookies.set("analyticsConsentAsked", "1"), 
...
version: 1647533457,
release: "Thu, 17 Mar 2022 16:10:57 GMT",
mathml_stylesheet: "/mathml.css",
favicon_spritesheet: "/assets/sprites/docs-f02127151f58a754274cfa422c3351e7e4abfec2a0001fac1f85b2e1346a9aac.png",
...
o = [ "count", "schema", "version", "news" ], t.prototype.LAYOUTS = [ "_max-width", "_sidebar-hidden", "_native-scrollbars", "_text-justify-hyphenate" ], 
t.defaults = {
count: 0,
hideDisabled: !1,
hideIntro: !1,
...
var n, o;
n = document.body.classList, "_sidebar-hidden" === t && null != (o = app.router) && o.isSettings || n.toggle(t, e), 
...
$.on($("._search"), "touchend", this.onTapSearch), this.toggleSidebar = $("button[data-toggle-sidebar]"), 
this.toggleSidebar.removeAttribute("hidden"), $.on(this.toggleSidebar, "click", this.onClickToggleSidebar), 
this.back = $("button[data-back]"), this.back.removeAttribute("hidden"), $.on(this.back, "click", this.onClickBack), 
